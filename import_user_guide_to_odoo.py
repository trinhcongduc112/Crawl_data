...
Based on the structure from docs.abivin.com on readme.io
"""
...
def get_article_category(doc: Dict) -> List[str]:
    """
    Xác định category cho article dựa trên title và content
    Returns: List of category path, e.g. ["User Guide", "Web App Tutorials", "VRP/DC Model"]
    """
    title = doc.get("title", "").lower()
    origin_url = doc.get("origin_url", "")
    order_index = doc.get("order_index", 999999)
    ...
    for keyword in faq_keywords:
        if any(kw in origin_url.lower() for kw in keyword.split('-')):
            # Determine FAQ subcategory
            if any(x in origin_url.lower() for x in ["customer", "customers"]):
                return ["User Guide", "FAQs", "Customer"]
            elif any(x in origin_url.lower() for x in ["product", "products"]):
                return ["User Guide", "FAQs", "Product"]
            elif any(x in origin_url.lower() for x in ["organization", "organizations"]):
                return ["User Guide", "FAQs", "Organization"]
            else:
                return ["User Guide", "FAQs", "General"]
    
    # Check Getting Started
    if any(kw in origin_url.lower() for kw in getting_started_keywords):
        return ["User Guide", "Getting Started"]
    
    # Web App Tutorials - VRP
    if "vrp" in origin_url.lower():
        if "dc" in origin_url.lower() or "depot" in origin_url.lower():
            return ["User Guide", "Web App Tutorials", "VRP/DC Model"]
        elif "in-house" in origin_url.lower():
            return ["User Guide", "Web App Tutorials", "VRP - In-House Fleet"]
        elif "outsourcing" in origin_url.lower():
            return ["User Guide", "Web App Tutorials", "VRP - Outsourcing Fleet"]
        else:
            return ["User Guide", "Web App Tutorials", "VRP/DC Model"]
    
    # Web App Tutorials - PDP
    if "pdp" in origin_url.lower() or "pick-delivery-pick" in origin_url.lower():
        if "in-house" in origin_url.lower() or "inhouse" in origin_url.lower():
    ...
    if "international-logistics" in origin_url.lower() or "freight-transport" in origin_url.lower():
        if "inland" in origin_url.lower():
    ...
    mobile_keywords = [
        "mobile-app",
        "mobile-app-general-instruction",
        "delivery-app",
        "consumer-app"
    ]
    if any(kw in origin_url.lower() for kw in mobile_keywords):
        if "vrp" in origin_url.lower() or "dc" in origin_url.lower():
            return ["User Guide", "Mobile App Tutorials", "VRP/DC Model"]
        elif "pdp" in origin_url.lower():
            return ["User Guide", "Mobile App Tutorials", "PDP Model"]
        elif "freight" in origin_url.lower() or "sea" in origin_url.lower() or "road" in origin_url.lower():
    ...
    if any(kw in origin_url.lower() for kw in wms_tms_keywords):
    ...
    if any(kw in origin_url.lower() for kw in policies_keywords):
    ...
    if any(kw in origin_url.lower() for kw in misc_keywords):
        return ["User Guide", "Miscellaneous Support"]
    
    # Route optimization and related
    if "route" in origin_url.lower() or "optimization" in origin_url.lower():
        if "map-view" in origin_url.lower():
            return ["User Guide", "Web App Tutorials", "Route Plan Optimization"]
        else:
            return ["User Guide", "Web App Tutorials"]
    
    # Default to Miscellaneous Support if no match
    return ["User Guide", "Miscellaneous Support"]
...
def import_user_guide():
    ...
    for idx, doc in enumerate(all_docs, 1):
        try:
            ...
            # Đảm bảo tất cả categories được tạo
            current_parent_id = space_id
            for category_name in category_path[:-1]:  # All except last
                ...
                if cache_key not in category_path_map:
                    ...
                    print(f"    ✓ Created/Found: {category_name} (ID: {cat_id})")
                else:
                    cat_id = category_path_map[cache_key]
                current_parent_id = cat_id
            
            # Category cho article (last in path)
            article_category_id = current_parent_id
            ...
            # 3. Tạo hoặc cập nhật article
            vals = {
                "name": title,
                "body": body,
                "parent_id": article_category_id
            }
            
            # Kiểm tra article đã tồn tại
            existing = odoo_search(
                models, uid, MODEL_ARTICLE,
                [("name", "=", title), ("parent_id", "=", article_category_id)],
                ["id"], limit=1
            )
    ...
    print(f"   User Guide với các categories: Getting Started, Web App Tutorials, Mobile App Tutorials, etc.")
    print("="*60)
