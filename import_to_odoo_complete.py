# =============================================================
# File: import_to_odoo_complete.py
# Import docs vào Odoo - Version Hoàn Hảo
# =============================================================
...
def load_all_docs() -> List[Dict]:
    ...
    content_dirs = [d for d in glob.glob(os.path.join(BASE_OUTPUT_DIR, "*", "content_*")) if os.path.isdir(d)]
    
    if not content_dirs:
        raise SystemExit(f"❌ LỖI: Không tìm thấy thư mục 'content_*' trong '{BASE_OUTPUT_DIR}'.")
    
    print(f"\n📂 Đang load dữ liệu từ {len(content_dirs)} sections...")
    
    for content_dir in content_dirs:
        section = os.path.basename(content_dir).replace("content_", "")
        assets_dir = os.path.join(os.path.dirname(content_dir), f"assets_{section}")
...
def import_all():
    ...
    print(f"\n{'='*60}")
    print("🔗 Đang thiết lập quan hệ cha-con...")
    print(f"{'='*60}\n")
    ...
    for doc in all_docs:
        try:
            ...
            if parent_slug and slug in article_map and parent_slug in article_map:
                article_id = article_map[slug]
                parent_id = article_map[parent_slug]
                
                try:
                    odoo_write(models, uid, MODEL_ARTICLE, [article_id], {
                        "parent_id": parent_id
                    })
                    parent_set_count += 1
                    if parent_set_count % 10 == 0:
                        print(f"  ✓ Đã set {parent_set_count} quan hệ cha-con")
                except Exception as e:
                    print(f"  ✗ Không thể set parent cho {slug}: {str(e)[:50]}")
        except Exception as e:
            print(f"  ✗ Lỗi khi set parent: {str(e)[:50]}")
    
    print(f"  ✓ Đã thiết lập {parent_set_count} quan hệ cha-con")
